...
class ABTestEngine:
    ...
    def generate_synthetic_experiment(self, df: pd.DataFrame, lift: float = 0.15) -> pd.DataFrame:
        """
    ...
    def analyze_experiment(self, df: pd.DataFrame) -> dict[str, Any]:
        """
        ...
        # 1. Relative Lift
        lift = (treatment["conversion_rate"] - control["conversion_rate"]) / control[
            "conversion_rate"
        ]
        ...
        se_a = np.sqrt(
            control["conversion_rate"] * (1 - control["conversion_rate"]) / control["visitors"]
        )
        se_b = np.sqrt(
        ...
        effect_size = (treatment["conversion_rate"] - control["conversion_rate"]) / np.sqrt(
            (
                control["conversion_rate"] * (1 - control["conversion_rate"])
                + treatment["conversion_rate"] * (1 - treatment["conversion_rate"])
            )
            / 2
        )
        power_analysis = TTestIndPower()
        power = power_analysis.solve_power(
            effect_size=effect_size, nobs1=control["visitors"], ratio=1.0, alpha=self.alpha
        )
...
def run_ab_simulation(cfg: Any) -> None:
    ...
    try:
        ...
        if results["stat_sig"] and results["lift"] > 0:
            logger.info(
                "RECOMMENDATION: SHIP IT! The coupon campaign drove significant incremental revenue."
            )
        else:
...
if __name__ == "__main__":
    # hydra is a CLI-only dependency of this script, imported lazily here so that
    # importing ABTestEngine elsewhere (e.g. the API's ABTestService) doesn't require
    # hydra/omegaconf to be installed.
    import hydra
