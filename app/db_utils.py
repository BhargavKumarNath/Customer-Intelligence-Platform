...
# Add src to path so the shared dimensional-model builder is importable
...
def get_connection():
    ...
    if IS_CLOUD or SAMPLE_MODE:
        ...
        try:
            ...
            # Build the star schema + ML prediction tables via the shared
            # builder also used by scripts/create_cloud_database.py, so the
            # in-process cloud rebuild and the offline cloud-DB build can't
            # drift apart (they previously carried two independent copies of
            # this SQL, including two independent copies of a recency bug
            # that anchored to CURRENT_DATE instead of the dataset's own
            # reference date).
            build_all(con)
